...
def voice_channel_player():
    def predicate(ctx: commands.Context):
        ...
        if not ctx.voice_client:
            em=discord.Embed(title="Error",description="Player is connected to any voice channel.")
            raise PlayerNotConnected(embed=em)

        if ctx.voice_client.channel.id != ctx.author.voice.channel.id:
            em=discord.Embed(title="Error",description="You must be in the same voice channel as the player.")
